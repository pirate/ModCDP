...
class BrowserbaseBrowserLauncher(BrowserLauncher):
    def launch(self, options: BrowserLaunchOptions | None = None) -> LaunchedBrowser:
        ...
        resume_session_id = _first_string(merged.get("session_id"), merged.get("resume_session_id"))
        keep_alive = _first_bool(merged.get("keep_alive")) or False
        close_session_on_close = _first_bool(merged.get("close_session_on_close"))
        ...
        if resume_session_id:
            ...
            session_create_params = _object_value(
                merged.get("session_create_params") or merged.get("browserbase_session_create_params")
            )
            browser_settings = {
                **_object_value(session_create_params.get("browserSettings")),
                **_object_value(merged.get("browser_settings")),
            }
            user_metadata = {
                **_object_value(session_create_params.get("userMetadata")),
                **_object_value(merged.get("user_metadata")),
            }
            ...
            region = _first_string(merged.get("region"), session_create_params.get("region"))
