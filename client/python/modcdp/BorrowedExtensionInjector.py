...
from .ExtensionInjector import DEFAULT_SERVICE_WORKER_POLL_INTERVAL_MS, DEFAULT_SERVICE_WORKER_PROBE_TIMEOUT_MS, DEFAULT_SERVICE_WORKER_READY_TIMEOUT_MS, EXT_ID_FROM_URL_RE, ExtensionInjector, ExtensionInjectionResult, MODCDP_READY_EXPRESSION
...
class BorrowedExtensionInjector(ExtensionInjector):
    def inject(self) -> ExtensionInjectionResult | None:
        ...
        while True:
            borrowed = self.borrowVisibleServiceWorkers()
            if borrowed:
                return borrowed
    ...
    def bootstrapTarget(self, target) -> ExtensionInjectionResult | None:
        session_id = self.ensureSessionIdForTarget(
            target["targetId"],
        ...
        try:
            ...
            pass
        bootstrap = self.sendWithTimeout(
            "Runtime.evaluate",
            {
        ...
        if ready and self.readyExpression() != MODCDP_READY_EXPRESSION:
