...
const existing_cdp = fs.existsSync(path.join(here, "cdp.ts")) ? fs.readFileSync(path.join(here, "cdp.ts"), "utf8") : "";
...
for (const base of modcdpCommands) {
  ...
  if (base === "AddCustomCommand") {
    ...
    lines.push(
      ...
      `          const eventSchema = normalizeModCDPPayloadSchema(directSchema.data);`,
      `          const response = Mod.${base}Response.parse(await send(${JSON.stringify(commandName)}, { name, eventSchema: null }));`,
...
const zod_dir = path.join(here, "zod");
...
console.log(
  `Generated types/cdp.ts, types/zod.ts, types/zod/*.ts, and types/aliases.ts from devtools-protocol@${meta.version} (${domains.length} domains).`,
);
