...
export type ExtensionInjectorConfig = {
  send?: SendCDP | null;
  sessionIdForTarget?:
    | ((target_id: string) => string | null | undefined)
    | null;
  attachToTarget?:
    | ((target_id: string) => Promise<string | null | undefined>)
    | null;
  waitForExecutionContext?:
    | ((session_id: string, timeout_ms: number) => Promise<number>)
    | null;
...
export class ExtensionInjector {
  ...
  getTransportConfig(): UpstreamTransportConfig {
    return this.options.injector_extension_id
      ? { injector_extension_id: this.options.injector_extension_id }
      : {};
  }
  ...
  protected async sendWithTimeout(
    ...
    timeout_ms = this.options.injector_cdp_send_timeout_ms ??
  ...
  protected async sessionIdForTarget(target_id: string, timeout_ms = 0) {
    ...
    while (true) {
      ...
      await delay(
        this.options.injector_target_session_poll_interval_ms ??
          DEFAULT_TARGET_SESSION_POLL_INTERVAL_MS,
      );
    }
  }

  protected async ensureSessionIdForTarget(
    target_id: string,
    timeout_ms = 0,
    allow_attach = false,
  ) {
    ...
    if (typeof session_id === "string" && session_id.length > 0)
      return session_id;
    if (allow_attach) {
      const attached_session_id =
        await this.options.attachToTarget?.(target_id);
      if (
        typeof attached_session_id === "string" &&
        attached_session_id.length > 0
      )
  ...
  protected async targetInfos() {
    return TargetCommands["Target.getTargets"].result.parse(
  ...
  protected async discoverReadyServiceWorker({
    ...
    if (this.options.injector_trust_service_worker_target || matched_only)
      return null;
  ...
  protected async waitForReadyServiceWorker(
    ...
    while (Date.now() < deadline) {
      ...
      await delay(
        this.options.injector_service_worker_poll_interval_ms ??
          DEFAULT_SERVICE_WORKER_POLL_INTERVAL_MS,
      );
    }
    return null;
  ...
  protected serviceWorkerTargetMatches(candidate: {
    ...
    if (
      ...
      !url.startsWith(
        `chrome-extension://${this.options.injector_extension_id}/`,
      )
    )
      return false;
